...
import { PATIENT_BLOOD_PRESSURE } from '../../constants';
import { DatePickerInput, DateTimePicker } from '@mantine/dates';
import { createUTCdateForISO } from '../../utils';
import useChildVitals from '../../hooks/use-child-vitals';
import empty from '../../assets/images/empty.svg';
import { showNotification } from '@mantine/notifications';
import { IBloodPressure, IChildrenResponse } from '../../api/types';

const ChildBloodPressure = ({ child, data }: { child: IChildrenResponse; data: IBloodPressure[] }) => {
	...
	const handleSubmit = async (data: any) => {
		await addChildVitals({
			...data,
			date: createUTCdateForISO(data.bloodPressure.date),
			childId: child.childId
		});
		showNotification({
			color: 'green',
			title: t('tr.success'),
			message: t('tr.pressure-added'),
		});
		setOpened(false);
		setQuery(' ');
		form.reset();
	...
	const form = useForm({
		...
		validate: {
			diastolicMmHg: (value) => (value < 1 ? t('tr.enter-diastolic') : null),
			systolicMmHg: (value) => (value < 1 ? t('tr.enter-systolic') : null),
			posture: (value) => (value.length < 1 ? t('tr.select-posture') : null),
				...
				>
					<Group grow={false} position="center">
						<DatePickerInput
							...
							radius="xl"
							aria-placeholder={t('tr.search-by-date')}
							onChange={(e) =>
								e
									...
									>
										...
										<Text ta={'center'} fz={12}>
											...
											<Badge color="teal.7" variant="outline" size="md">
												{fixedDate(bloodInfo.date).slice(4, 16)}
											</Badge>
										</Text>
									</Flex>
								</Accordion.Control>
										...
										>
											<div>
												<TextInput
													...
													value={`${bloodInfo.diastolicMmHg} mm Hg`}
													placeholder={`${bloodInfo.diastolicMmHg} mm Hg`}
												/>
												<Space h="xs" />
											</div>
											<div>
												<TextInput
													...
													value={`${bloodInfo.systolicMmHg} mm Hg`}
													placeholder={`${bloodInfo.systolicMmHg} mm Hg`}
												/>
												<Space h="xs" />
											</div>
											<div>
												<TextInput
													...
													label={
														<>
															<Text mb={5}>{t('tr.posture')}</Text>
														</>
													}
													value={bloodInfo.posture}
												/>
											</div>
											<div>
												<TextInput
													...
													value={fixedDate(bloodInfo.date)}
												/>
											</div>
										</SimpleGrid>
									</Paper>
				...
				<Fragment>
					<form
						onSubmit={form.onSubmit((values) =>
							handleSubmit({ bloodPressure: values })
						)}
					>
						<div style={{ margin: '0 auto' }}>
							<h1 style={{ margin: '1rem' }} className={styles.title}>
								{t('tr.addyour-blood-pressure')}
							</h1>
						</div>
								...
								>
									<div>
										<NumberInput
											...
											min={0}
											max={250}
											placeholder={child?.polygenic.diastolic}
											{...form.getInputProps('diastolicMmHg')}
										/>
										<Space h="xs" />
										<Slider
											px={5}
									...
									<div>
										<NumberInput
											...
											max={250}
											placeholder={child?.polygenic.diastolic}
											{...form.getInputProps('systolicMmHg')}
										/>
										<Space h="xs" />
										<Slider
											px={5}
									...
									<div>
										<Select
											...
											mb={'sm'}
											defaultValue={child?.polygenic.posture}
											label={
												<>
													<Text mb={5}>{t('tr.posture')}</Text>
												</>
											...
											data={[
												{ value: 'Sitting', label: t('tr.sitting') },
												{ value: 'Standing', label: t('tr.standing') },
												{ value: 'LayingDown', label: t('tr.layingdown') },
											]}
											{...form.getInputProps('posture')}
										/>
									</div>
									<div>
										<DateTimePicker
...
export default ChildBloodPressure;
