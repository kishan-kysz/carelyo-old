...
import { PATIENT_PULSE } from '../../constants';
import { DatePickerInput, DateTimePicker } from '@mantine/dates';
import { createUTCdateForISO } from '../../utils';
import empty from '../../assets/images/empty.svg';
import { showNotification } from '@mantine/notifications';
import { IChildrenResponse, IHeartRate } from '../../api/types';
import useChildVitals from '../../hooks/use-child-vitals';

const ChildPulse = ({ child, data }: { child: IChildrenResponse, data: IHeartRate[] }) => {
	...
	const handleSubmit = async (data: any) => {
		await addChildVitals({
			...data,
			date: createUTCdateForISO(data.heartRate.date),
			childId: child.childId
		});
		showNotification({
			color: 'green',
			title: t('tr.success'),
			message: t('tr.pulse-successfully-added'),
		});
		setOpened(false);
		setQuery(' ');
		form.reset();
	...
	const filteredData = data?.filter((pulseInfo) =>
		fixedDate(pulseInfo.date)
			.slice(4, 16)
			.toLowerCase()
			.toString()
	...
	const form = useForm({
		...
		validate: {
			heartRateBpm: (value) => (value < 1 ? t('tr.pleaseenter-pulse') : null),
									...
									>
										...
										<Text ta={'center'} fz={12}>
											...
											<Badge color="teal.7" variant="outline" size="md">
												{fixedDate(heartRate.date).slice(4, 16)}
											</Badge>
										</Text>
									</Flex>
								</Accordion.Control>
										...
										>
											<div>
												<TextInput
													...
													value={`${heartRate.heartRateBpm} Bpm`}
												/>
											</div>
											<div>
												<TextInput
													...
													value={fixedDate(heartRate.date)}
												/>
											</div>
										</SimpleGrid>
									</Paper>
								...
								>
									<div>
										<NumberInput
											...
											min={0}
											max={250}
											placeholder={child?.polygenic.pulse}
											{...form.getInputProps('heartRateBpm')}
										/>
										<Space h="xs" />
										<Slider
											px={5}
